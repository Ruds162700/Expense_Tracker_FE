    ...
    >
      ...
      {expanded && (
        <Box sx={{ p: 2, bgcolor: 'white' }}>
          <Typography variant="subtitle2" sx={{ color: '#1976d2', mb: 2, fontWeight: 'bold' }}>
            Split Details
          </Typography>
            ...
            {expense.details.map((detail, index) => (
              <Grid item size = {{xs:12 , sm:6 }} key={index}>
                <Box sx={{ 
                  ...
                  alignItems: 'center',
                  p: 1.5,
                  bgcolor: detail.contribution > 0 ? '#f1f8e9' : '#fff3e0',
                  borderRadius: 1,
                  border: '1px solid',
                  borderColor: detail.contribution > 0 ? '#c5e1a5' : '#ffe0b2'
                }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Avatar sx={{ 
                      width: 24, 
                      height: 24, 
                      bgcolor: detail.contribution > 0 ? '#4caf50' : '#ff9800'
                    }}>
                      <PersonIcon sx={{ fontSize: 16 }} />
                    </Avatar>
                    <Box>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        {detail.contribution > 0 ? 'Paid' : detail.owe > 0 ? 'Owes' : 'Gets Back'}
                      </Typography>
                    </Box>
                  </Box>
                  <Box>
                    {detail.contribution > 0 && (
                      <Typography variant="body2" sx={{ color: '#2e7d32', fontWeight: 'bold' }}>
                        Paid: ₹{detail.contribution.toLocaleString('en-IN')}
                      </Typography>
                    )}
                    {detail.owe > 0 && (
                      <Typography variant="body2" sx={{ color: '#f57c00', fontWeight: 'bold' }}>
                        Owes: ₹{detail.owe.toLocaleString('en-IN')}
                      </Typography>
                    )}
                    {detail.debt > 0 && (
                      <Typography variant="body2" sx={{ color: '#d32f2f', fontWeight: 'bold' }}>
                        Gets back: ₹{detail.debt.toLocaleString('en-IN')}
                      </Typography>
                    )}
                  </Box>
                </Box>
...
const GroupExpensePage = () => {
  ...
  const handleEditClick = (expense) => {
    ...
    const formattedExpense = {
      ...
      category: expense.category || expense.expenses_category,
      date: expense.date || expense.expenses_date,
